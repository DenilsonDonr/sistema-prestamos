...
async function getAll() {
  const [rows] = await db.query(
    'SELECT id, nombre, descripcion, activo, created_at, updated_at FROM areas ORDER BY nombre ASC'
  );
  return rows;
}
...
async function getById(id) {
  const [rows] = await db.query(
    'SELECT id, nombre, descripcion, activo, created_at, updated_at FROM areas WHERE id = ?',
    [id]
  );
...
/**
 * Crea una nueva área.
 * @throws {409} Si ya existe un área con ese nombre.
 */
async function create(fields) {
  ...
  if (existing.length > 0) {
    const err = new Error(`Ya existe un área con el nombre "${fields.nombre}"`);
    err.statusCode = 409;
  ...
  const [result] = await db.query(
    `INSERT INTO areas (${columns}) VALUES (${placeholders})`,
...
/**
 * Baja lógica: marca el área como inactiva (activo = false).
 * @param {number} id
 * @param {number|null} usuarioModificaId
 */
async function remove(id, usuarioModificaId) {
  ...
  await db.query(
    'UPDATE areas SET activo = false, usuario_modifica_id = ?, updated_at = NOW() WHERE id = ?',
